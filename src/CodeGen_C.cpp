...
// Failsafe to allow forcing non-native vectors in case of unruly compilers
    ...
    #define halide_cpp_use_native_vector(type, lanes) (false)
        ...
        void CodeGen_C::generate_function_declaration_for_single_kernel_for_AI(
            ...
            for (unsigned i = 0; i < buffer.size(); i++) {
                string buffer_name = print_name(buffer[i].name()) + "_buffer";
                stream << print_type(buffer[i].type(), AppendSpace) +  " * restrict " << buffer_name;
                for(int j = 0; j < buffer[i].dimensions(); j++) {
                    stream << ", int " << (buffer_name + "_dim" + std::to_string(j) + "_min");
                    stream << ", int " << (buffer_name + "_dim" + std::to_string(j) + "_extent");
                    stream << ", int " << (buffer_name + "_dim" + std::to_string(j) + "_stride");
                }
            ...
            for (unsigned i = 0; i < f.args.size(); i++) {
                if (f.args[i].is_buffer()) {
                    string buffer_name = print_name(f.args[i].name) + "_buffer";
                    stream << print_type(f.args[i].type, AppendSpace) << " * restrict " << buffer_name;
                    for(int j = 0; j < f.args[i].dimensions; j++) {
                        stream << ", int " << (buffer_name + "_dim" + std::to_string(j) + "_min");
                        stream << ", int " << (buffer_name + "_dim" + std::to_string(j) + "_extent");
                        stream << ", int " << (buffer_name + "_dim" + std::to_string(j) + "_stride");
                    }
                } else {
                    stream << print_type(f.args[i].type, AppendSpace)
                           << print_name(f.args[i].name);
                }
        ...
        void CodeGen_C::compile(const Module &input) {
            ...
            if (!is_header()) {
                ...
                if(input.target().arch != Target::AI)
            ...
            if(input.target().arch != Target::AI) {
                ...
                assert(input.functions().size() == 1);
                generate_function_declaration_for_single_kernel_for_AI(input.functions()[0], input.buffers());
            }
        }
